...
bool test_main(void) {
  ...
  uint32_t reg_control =
      bitfield_field32_write(0u, KEYMGR_CONTROL_SHADOWED_DEST_SEL_FIELD,
  ...
  // Write CONTROL_SHADOWED (shadowed register write) and then START
  // (non-shadowed)
  ...
  if (op_status == KEYMGR_OP_STATUS_STATUS_VALUE_DONE_ERROR) {
    ...
    uint32_t reg_state =
        mmio_region_read32(keymgr, KEYMGR_WORKING_STATE_REG_OFFSET);
    uint32_t state_field =
        bitfield_field32_read(reg_state, KEYMGR_WORKING_STATE_STATE_FIELD);
    ...
    CHECK(state_field == KEYMGR_WORKING_STATE_STATE_VALUE_INVALID,
          "Expected keymgr to enter INVALID state due to truncated assignment "
          "(bug)");

    LOG_INFO(
        "POC detected ECC/truncation failure leading to INVALID state - test "
        "PASS");
    return true;
  }

  // If the operation completed successfully, the bug was not triggered in this
  // run.
  LOG_INFO("Keymgr operation completed successfully (OP_STATUS=0x%08x).",
           op_status);
  CHECK(false,
        "Bug not triggered: keymgr did not report ECC error/INVALID state");
  return false;
}
