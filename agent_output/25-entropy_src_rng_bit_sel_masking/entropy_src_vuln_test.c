...
static uint32_t htwin_read(void) {
  return mmio_region_read32(entropy_base, ENTROPY_SRC_HEALTH_TEST_WINDOW_REG_OFFSET);
}

static void htwin_write(uint32_t v) {
  mmio_region_write32(entropy_base, ENTROPY_SRC_HEALTH_TEST_WINDOW_REG_OFFSET, v);
}
