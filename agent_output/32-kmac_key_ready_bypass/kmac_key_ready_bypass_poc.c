...
bool test_main(void) {
  ...
  // We set the mode to KMAC and choose a valid strength value. This mirrors
  // the steps taken by DIF but is done directly through MMIO to keep the POC
  // low-level and deterministic.
  uint32_t cfg_reg = mmio_region_read32(kmac_base, KMAC_CFG_SHADOWED_REG_OFFSET);

  // Set K strength to 128 (use provided constant) and mode to KMAC.
  ...
  cfg_reg = bitfield_field32_write(cfg_reg, KMAC_CFG_SHADOWED_MODE_FIELD,
                                   KMAC_CFG_SHADOWED_MODE_VALUE_KMAC);
  ...
  LOG_INFO("Wrote KMAC CFG (mode=KMAC, kstrength=128). Not programming key shares.");
