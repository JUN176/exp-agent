...
bool test_main(void) {
  ...
  if (force_masks_set) {
    LOG_INFO("FORCE_MASKS bit is already set (vulnerable: software can
force masks at runtime)");
    // We still assert that software can read the bit and observe it set.
    CHECK(force_masks_set, "Expected FORCE_MASKS to be set");
    return true;
  }

  LOG_INFO("FORCE_MASKS bit is not set. Attempting to enable it via
software...");
  ...
  CHECK(force_masks_after,
        "Failed to enable FORCE_MASKS via CSR path - target may not be
vulnerable or register is locked");

  LOG_INFO("Successfully set FORCE_MASKS via CSR path. AES masking can be
forced to constant values by software (vulnerable)");
