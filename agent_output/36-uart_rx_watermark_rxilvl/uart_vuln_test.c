...
// Generated register header for UART (provided by //hw/top:uart_c_regs)
#include "uart_regs.h"
...
bool test_main(void) {
  mmio_region_t uart = mmio_region_from_addr(TOP_EARLGREY_UART_BASE_ADDR);
  ...
  // Try to send up to 32 bytes (the implemented FIFO size). Stop if TX becomes full.
  const uint32_t kFifoSize = 32u;
  uint32_t bytes_sent = 0u;
  while (bytes_sent < kFifoSize) {
